...
export default async function UsersPage() {
  ...
  container.innerHTML = `
    <div class="pg-hdr">
      <div>
        <h1>Users</h1>
        <div class="pg-sub">Manage user accounts</div>
      </div>
    ...
    <div class="filters">
      <div class="search-box">
        <span class="si">🔍</span>
        <input placeholder="Search users..." id="search-input">
      </div>
    </div>
    <div id="users-table"></div>
  `;

  const actionsEl = container.querySelector('#pg-actions');
  ...
  async function loadUsers() {
    const tableContainer = container.querySelector('#users-table');
  ...
  function showEditModal(user) {
    ...
    body.innerHTML = `
      <form id="edit-user-form">
        <div class="form-g">
          <label class="form-l">Email <span class="req">*</span></label>
          <input type="email" name="email" class="form-i" value="${user.email}" required>
        </div>
        <div class="modal-error" id="modal-error" style="display:none;"></div>
      </form>
    `;
    ...
    const submitBtn = Button({
      ...
      onClick: async () => {
        ...
        try {
          await usersApi.update(user.user_id, { email });
          modal.close();
          loadUsers();
        } catch (err) {
          errorEl.textContent = err.message;
