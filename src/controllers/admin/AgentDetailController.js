...
export default async function AgentDetailController(params) {
  ...
  function renderViewSkills(list) {
    ...
    list.forEach(function(s) {
      ...
      r.innerHTML = '<span style="font-size:18px">📄</span><div style="flex:1"><div class="text-sm fw-m">' + s.skill_name + '</div><div class="text-xs text-t mt-1">' + (s.file_name || '') + (s.description ? ' · ' + s.description : '') + '</div></div>';
      c.appendChild(r);
    });
  }
  ...
  function renderEditSkills(list) {
    ...
    list.forEach(function(s) {
      var r = document.createElement('div');
      r.style.cssText = 'display:flex;align-items:center;gap:12px;padding:12px 20px;border-bottom:1px solid var(--gray-100)';
      r.innerHTML = '<span style="font-size:18px">📄</span><div style="flex:1"><div class="text-sm fw-m">' + s.skill_name + '</div><div class="text-xs text-t mt-1">' + (s.file_name || '') + '</div></div><button class="btn btn-d btn-sm" data-detach-skill="' + s.skill_id + '">Detach</button>';
      c.appendChild(r);
    });
  }
  ...
  el.querySelector('[data-action="addSkill"]').addEventListener('click', function() {
    ...
    if (!available.length) {
      ...
      available.forEach(function(s) {
        ...
        lbl.innerHTML = '<input type="checkbox" value="' + s.skill_id + '" style="width:16px;height:16px"> <div><div class="fw-m">' + s.skill_name + '</div><div class="text-xs text-t">' + (s.file_name || '') + '</div></div>';
        cbs.appendChild(lbl);
      });
    }
