...
export default async function ProjectDetailController(params) {
  ...
  function renderKbDocs() {
    ...
    container.innerHTML = '<table style="width:100%;border-collapse:collapse"><thead><tr>'
      ...
      + kbDocuments.map((doc, i) => {
          ...
          return '<tr>'
            + '<td style="padding:12px 20px;' + border + '"><div class="flex items-c gap-3">' + getIcon(doc.file_name) + ' <a href="#" class="text-sm fw-m text-pri" style="text-decoration:none" data-action="kbDownload" data-s3-key="' + (doc.s3_key || '') + '">' + sanitize(doc.file_name || '—') + '</a></div></td>'
            + '<td style="padding:12px 20px;font-size:12px;color:var(--gray-500);' + border + '">' + sanitize(doc.description || '—') + '</td>'
  ...
  function renderSessionsTab() {
    ...
    tbody.innerHTML = sessions.map(s => {
      ...
      return '<tr><td><strong class="text-p">' + sanitize(s.name || 'Untitled') + '</strong><div class="text-xs text-t">' + startTimeDisplay + '</div></td><td>' + getStatusBadge(uiStatus) + '</td><td class="mono text-xs">' + meetLinkHtml + '</td><td>—</td><td class="mono text-xs">' + duration + '</td><td><div class="session-action-btns">' + actionHtml + '</div></td></tr>';
    }).join('');
  ...
  async function loadSessions() {
    ...
    try {
      ...
      sessionsError.innerHTML = '<div class="empty"><div class="empty-icon">⚠️</div><div class="empty-title">Failed to load sessions</div><div class="empty-desc">' + sanitize(err.message) + '</div></div>';
    }
  }
        ...
        <div class="tbl-wrap">
          <table>
            ...
            <tbody>
              ${assignedUsers.map(u => `
                <tr>
                  <td><strong>${sanitize(u.username || u.email)}</strong></td>
    ...
    bodyEl.innerHTML = `
      <div class="form-g">
        ...
        <select class="form-sel" id="assignUserSelect">
          ...
          ${available.map(u => `<option value="${u.user_id}">${sanitize(u.username || u.email)} (${sanitize(u.email)})</option>`).join('')}
        </select>
      </div>
