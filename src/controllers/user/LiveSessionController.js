...
export default async function LiveSessionController() {
  ...
  function handleQuestionMatched(data) {
    ...
    notification.innerHTML = '<div class="card-body" style="padding:12px 16px">'
      ...
      + '<div class="text-sm fw-m text-p mb-2">Suggested: ' + (data.question_text || '') + '</div>'
      + '<div class="text-sm text-s mb-2">Spoken: "' + (data.spoken_text || '') + '"</div>'
  ...
  function handleGapAnalysis(data) {
    ...
    let html = gaps.map(gap =>
      ...
      + '</div>'
      + '<div class="text-sm fw-m text-p mb-2">' + (gap.topic || '') + '</div>'
      + '<div class="text-xs text-t">' + (gap.description || '') + '</div>'
      + '</div></div>'
    ).join('');
    ...
    if (suggestedQs.length > 0) {
      html += '<div class="card mb-4" style="border-color:var(--pri-200);background:var(--pri-25)">'
        ...
        + suggestedQs.map(q => '<div class="text-sm text-p" style="padding:4px 0;border-bottom:1px solid var(--pri-100)">' + q + '</div>').join('')
        + '</div></div>';
    }
  ...
  function handleStatusMessage(data) {
    ...
    overlay.innerHTML = '<div style="text-align:center"><div class="loading-spinner" style="margin:0 auto 12px"></div><div class="text-md fw-sb">' + message + '</div></div>';
